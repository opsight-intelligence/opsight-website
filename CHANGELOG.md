...
### Changed
- The hackathon credit names the event: Snowflake APJ Hackathon 2026, 2nd place (en + ko).
...
### Added
- About gains *Who builds it*: four credits stated as résumé lines — senior solution engineer in manufacturing who worked directly with Fortune 500 OEMs and tier-1 suppliers (no employer named), 2nd place in the 2026 APJ data-platform hackathon for an automated manufacturing workflow, the Korean procurement line, OSINT entity classification. En + ko.
...
### Added
- **OpSentry in Korean** at `/ko/opsentry`, so every page on the site is
  English + Korean; the fraud page additionally keeps Turkish for its
  Turkish buyers. Product names, commands, statute and standard names stay
  as they are. Written by the agent; needs a native read.
